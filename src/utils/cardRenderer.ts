...
const getStarterTips = (type: string): string => {
  switch (type) {
    case 'group_payments':
      return `
        • Say "Create group payment for Alice with 5 people for 100 FLOW"<br>
        • Or try "Make a group payment for dinner - 50 FLOW for 4 friends"<br>
        • Split costs easily and track contributions in real-time
      `
    case 'savings_pots':
      return `
        • Say "Create savings pot 'Vacation' with target 1000 FLOW"<br>
        • Or try "Make a savings goal for new laptop - 500 FLOW"<br>
        • Set goals and track your progress automatically
      `
    default:
      return `
        • Say "Send 10 FLOW to Alice" to start transferring<br>
